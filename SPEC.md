...
- **Satu modul**:
  ...
  ```
- **Status saat ini**: **195 test lulus**.
...
- **Coverage**: **100%** (branch coverage via `coverage`, lihat `.coveragerc`; `source = Sastrawi`, `branch = True`). Seluruh statement & branch ter-cover, termasuk getter interface, cabang defensif ECS (`loop_pengembalian_akhiran`), guard `C == 'r'` pada Rule 3/7/9/24, dan guard `stem_plural_word`.
